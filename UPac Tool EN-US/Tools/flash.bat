...
if exist "%package_path%\" (
    ...
    if /i not "%package_path:~-3%"==".7z" (
        echo Error: Only .7z format compressed packages are supported
        pause
        goto load_package
    )
...
ECHO.If stuck at 'waiting dl_diag', try reconnecting the device or check drivers (15 minutes remaining)
cd "%dump_files%"
...
ECHO.Formatting data using F2FS...
adb shell mkfs.f2fs /dev/block/by-name/userdata
if errorlevel 1 (
...
ECHO.Format complete, restarting device...
adb reboot recovery
ECHO.Waiting for device to enter Recovery...
adb wait-for-recovery >nul 2>&1
...
:push_retry
adb push "%push_path%\." /sdcard/
adb push "%bin_path%\.twrps" /sdcard/TWRP/
if errorlevel 1 (
    echo Device not connected, or failed to push files, retrying...
    if not defined UNATTENDED pause
    timeout /t 5 /nobreak >nul
    goto push_retry
)
...
:flash_vendor_retry
adb shell "dd if=/sdcard/vendor.bin of=%vendor_path%/vendor"
if errorlevel 1 (
    echo Failed to flash vendor partition, retrying...
    if not defined UNATTENDED pause
    timeout /t 5 /nobreak >nul
...
:restore_retry
adb shell twrp restore /sdcard/TWRP/BACKUPS/%backup_name% SDRB
if errorlevel 1 (
    echo Failed to restore system, or device not connected, retrying...
    if not defined UNATTENDED pause
    timeout /t 5 /nobreak >nul
    goto restore_retry
)
...
:delete_backup_retry
adb shell rm -rf /sdcard/TWRP/BACKUPS/%backup_name%
if errorlevel 1 (
...
:delete_vendor_retry
adb shell rm -f /sdcard/vendor.bin
if errorlevel 1 (
...
ECHO.The device will boot into the %system% system...
pause
goto start